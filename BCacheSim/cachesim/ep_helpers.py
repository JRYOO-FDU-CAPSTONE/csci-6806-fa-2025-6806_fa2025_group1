...
def _lookup_episode(decisions, block_id, ts, *, chunk_id=None, prune_old=False):
    ...
    if isinstance(decs[0], tuple):
        ...
        decs = tuple(
            Episode(*eps_args, **filter_dct(eps_kwargs))
    ...
    for episode in decs:
        ...
        if (
            ...
            and episode.chunk_level
            and not episode.contains(chunk_id)
        ):
            continue
        ...
        if eps_ts[0] <= ts.physical and (
            result is None or eps_ts[0] > result.ts_physical[0]
        ):
            result = episode
    if result is None:
        raise Exception(
            f"Error: Episode not found: (block_id, ts, chunk_id) ({block_id}, Timestamp{ts}, chunk_id={chunk_id})"
        )
    if prune_old:
...
def _prefetchable_chunks(
    decisions, block_id, ts_prefetch, assumed_ea=None, threshold=None
):
    if isinstance(decisions[block_id][0], tuple):
        decisions[block_id] = tuple(
            Episode(*eps_args, **eps_kwargs)
    ...
    for episode in decisions[block_id]:
        ...
        to_prefetch = (
            assumed_ea is not None
            and assumed_ea.physical != 0
            and ts_prefetch.physical < eps_ts[0]
            and ts_prefetch.physical + assumed_ea.physical > eps_ts[0]
        )
        to_prefetch = to_prefetch or (
            eps_ts[0] - episode.s_export["time_from_prefetch"][1]
            <= ts_prefetch.physical
            <= eps_ts[0]
        )
        if to_prefetch:
            for chunk_id in range(*episode.chunk_range):
...
class Timestamp(namedtuple("Timestamp", ["logical", "physical"])):
    ...
    def __sub__(self, other):
        ...
        return Timestamp(self.logical - other.logical, self.physical - other.physical)
...
def record_service_time_get(need_fetch, need_prefetch, acc):
    ...
    if need_prefetch:
        range_prefetch = (
            min(range_prefetch[0], min(need_prefetch)),
            max(max(need_prefetch), range_prefetch[1]),
        )
    ...
    ods.bump(
        "service_time_used_prefetch", v=service_time(0, num_with_prefetch - num_fetch)
    )
    ...
    for tag in tags:
        ...
        ods.bump(
            ["fetches", "chunks", "prefetch", tag], v=num_with_prefetch - num_fetch
        )
        ...
        ods.bump(
            ["service_time", "prefetch", tag],
            v=service_time(0, num_with_prefetch - num_fetch),
        )
    ...
    ods.bump(
        "service_time_used2_prefetch",
        v=service_time(0, num_with_prefetch2 - num_fetch2),
    )
    ...
    ods.bump(
        "service_time_used3_prefetch",
        v=service_time(0, num_with_prefetch3 - num_fetch3),
    )
...
def record_service_time_put(acc):
    ...
    ods.bump(
        ["service_time_writes", "op", acc.features.op.name],
        v=service_time(1, acc.num_chunks),
    )
