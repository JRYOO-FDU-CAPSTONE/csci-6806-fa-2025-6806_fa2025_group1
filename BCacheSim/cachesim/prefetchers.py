...
class Prefetcher(object):
    def __init__(self, *, options=None):
        ...
        if "episode-predict" in self.pf_range or "predict" in self.pf_when:
            if self.pf_when == "predict":
                ...
                print(
                    f"LearnedRangeConfPrefetcherModel(threshold={options.prefetch_when_threshold}, path={options.prefetcher_model_path})"
                )
            else:
                self.model = LearnedRangePrefetcherModel(options.prefetcher_model_path)
    ...
    def set_cache(self, *, cache=None, ram_cache=None, insert_cache=None, ap=None):
        ...
        self.insert_cache = (
            insert_cache if insert_cache else (ram_cache if ram_cache else cache)
        )
        self.ap = ap
        ...
        if self.pf_when.startswith("rejectfirst"):
            ...
            if self.pf_when == "rejectfirst-either":
                ...
                if block_id in self.ram_cache.cached_episodes:
                    iops_misses += self.ram_cache.cached_episodes[block_id][
                        "iops_misses"
                    ]
            if not in_cache:
    ...
    def get_chunks(self, block_id, acc_ts, episode, pred_prefetch, size, chunk_range):
        ...
        if self.pf_range.startswith("chunk") or self.pf_range.startswith("acctime-"):
            ...
            if self.pf_range == "chunk":
                ...
                chks, chk_episodes = _prefetchable_chunks(
                    cache.episodes, block_id, acc_ts, assumed_ea=self.assumed_ea
                )
                metadata_chks = {}
                for chk in chks:
                ...
                if "--fast" not in sys.argv:
                    ...
                    if episode:
                        cache.bump_counter(
                            "loss_prefetch_start",
                            predict_stats["chunk_r"][0] - episode.chunk_range[0],
                        )
                        cache.bump_counter(
                            "loss_prefetch_end",
                            predict_stats["chunk_r"][1] - episode.chunk_range[1],
                        )
            else:
                # REQUIRES: episode
                prefetch_size = utils.BlkAccess.MAX_BLOCK_SIZE
                if episode:
                    metadata_init["at_ep_start"] = (
                        episode.ts_physical[0] == acc_ts.physical
                    )
                    prefetch_size = max(prefetch_size, episode.size)
                if self.pf_range == "acctime-all":
                    acc_ = utils.BlkAccess(
                        0, prefetch_size, acc_ts.physical, block=block_id
                    )
                    chks = acc_.chunks()
                else:
    ...
    def filter_ap(self, chks, acc, metadata):
        ...
        decisions = self.ap.batchAccept(
            admit_buffer_, acc.ts, metadata=metadata_, check_only=True
        )
    ...
    def run(self, acc: AccessPlus, is_hit, chunk_hit, episode, misses, size):
        need_prefetch = []
        if self.decide(
            acc.block_id, is_hit, chunk_hit, episode, acc.pred_prefetch, acc.ts
        ):
            chks, metadata_chks = self.get_chunks(
                acc.block_id, acc.ts, episode, acc.pred_prefetch, size, acc.chunk_range
            )
            ...
            if chks:
                ...
                for chunk_id in need_prefetch:
                    ...
                    self.insert_cache.insert(
                        k, acc.ts, featvec, metadata=metadata_chks[chunk_id]
                    )
        return need_prefetch
...
class LearnedRangePrefetcherModel(PrefetcherModel):
    def __init__(self, path, keys=["offset_start", "offset_end", "size"]):
        ...
        self.models = {k: lgb.Booster(model_file=path.format(k=k)) for k in self.keys}
    ...
    def predict(self, features, metadata=None):
        ...
        if "--log-prefetch" in sys.argv:
            ...
            print(
                f"PREFETCH {start} {end} ({preds['offset_start']} {preds['offset_end']} {preds['size']})"
            )
    ...
    def predict_batch(self, features):
        ...
        for s, e in ranges:
            assert 0 <= s <= e
        chunks_rs = [offset_to_chunks(start, end) for start, end in ranges]
        return [(list(range(*cr)), {"chunk_r": cr}) for cr in chunks_rs]


class LearnedRangeConfPrefetcherModel(LearnedRangePrefetcherModel):
    def __init__(
        self, path, *, e_keys=["pred_net_pf_st_binary"], threshold=None, **kwargs
    ):
        self.e_keys = e_keys
    ...
    def predict(self, features, metadata=None):
        ...
        # Add to X
        preds = {k: self.models[k].predict(features_with_preds) for k in self.e_keys}
    ...
    def predict_batch(self, features):
        ...
        preds_ = {k: self.models[k].predict(features_with_preds) for k in self.e_keys}
        ods.bump("ml_batches", v=len(self.e_keys))
