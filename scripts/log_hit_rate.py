...
def get_parser():
    ...
    parser.add_argument(
        ...
        "--trace",
        type=str,
        default="data/tectonic/201910/Region1/full_0_0.1.trace",
        help="Specify the trace file to evaluate",
    )
...
def check_tracefile(args):
    trace_path = args.trace

    if not os.path.exists(trace_path):
        print(
            f"Error: Trace file '{trace_path}' not found for {args.policy} policy.",
            file=sys.stderr,
        )
        print(
            f"Please ensure the trace file exists at: {os.path.abspath(trace_path)}",
            file=sys.stderr,
        )
        sys.exit(1)
    else:
        print(f"\033[32m✔\033[0m Using trace file: \033[32m{trace_path}\033[0m")
...
def main():
    ...
    def spinner_wrapper():
        elapsed_time[0] = show_spinner(
            stop_spinner, f"Running eviction simulation for {policy}"
        )
    ...
    peak_dt = stats["results"]["ChunkHitRatio"] * 100
    print(f"\033[33m●\033[0m Cache Hit Rate: {peak_dt:.2f} %")
