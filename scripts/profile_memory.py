...
def main():
    ...
    # Start spinner and memory monitoring
    stop_spinner = threading.Event()
    elapsed_time = [0]

    def spinner_wrapper():
        elapsed_time[0] = show_spinner(
            stop_spinner, f"Running memory profiling for {policy}"
        )
    ...
    try:
        ...
        mem_usage = memory_usage(
            ...
            max_usage=True,  # Only return peak
        )
    finally:
        # Restore
        sys.stdout.close()
        ...
        # Stop spinner and memory monitor
        stop_spinner.set()
        spinner_thread.join()
    ...
    print(f"\033[33m●\033[0m Peak memory: {mem_usage:.2f} MB")
